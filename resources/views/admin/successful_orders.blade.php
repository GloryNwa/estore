        ...
        <div class="filter-items">
            <div class="row grid-wrap mrg20">
                ...
                <form class="topbar-search"method="POST" action="{{url('/search')}}">
                 ...
                 <h4 class="widget-title">Successful Orders</h4>
                           ...
                           <?php $x = 1;?>
                             @foreach($status as $statu)
                                    <tr>
                                         ...
                                         <td>
                                            <h4 class="name" style=" font-size: 17px ">{{$statu->name}}</h4>
                                        </td>
                                        <td>
                                            <span class="ph#" style=" font-size: 17px" >{{$statu->email}}</span>
                                        </td>
                                         
                                        <td>
                                            <span class="addr" style=" font-size: 17px ">{{$statu->phone}}</span>
                                        </td>
                                        
                                       <td>
                                            <h4 class="name"style=" font-size: 17px ">{{$statu->church}}</h4>
                                        </td>
                                        <td style=" font-size: 17px">
                                            <?php 
                                            //Explode array using the separating comma to convert string to an array 
                                                $exploded_services = explode(',',$statu->services);
                                             ?>
                                             ...
                                             @foreach($exploded_services as $services)
                                                ...
                                                //Fetch product details from database based on the id from array string
                                                    $statu_details = \App\Product::where("id",$services)->first(); 

                                           ?>
                                             <span class="addr alert alert-info"style="font-size: 17px">{{$statu_details->title}}</span> <br />
                                            @endforeach

                                        </td>
                                         <td>
                                            <span class="ph#" style=" font-size: 17px" >{{$statu->amountbox}}</span>
                                        </td>
                                        <td>
                                            <span class="ph#"style=" font-size: 17px;" >{{$statu->currency}}</span>
                                        </td>
                                        <td>

                                         <span class="date" style="color: green; font-size: 17px">{{$statu->status}}</span>
                                             <i class="fa fa-check" aria-hidden="true" style="color: green;"></i>
                                        </td>
                                         
                                        <td>
                                            <span class="addr" style="color: ;font-size: 17px ">{{$statu->created_at}}</span>
                                        </td>
