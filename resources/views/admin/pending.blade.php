                           ...
                           <?php $x = 1;?>
                             @foreach($orders as $order)
                                  <tr>
                                         ...
                                         <td>
                                            <h4 class="name" style="color: gray; font-size: 17px">{{$order->name}}</h4>
                                             <a href='{{url("/edit_orders/{$order->id}")}}'>
                                             <span class="fa fa-pencil" style="color:#009efb;"> Edit</span></a>  
                                        </td>
                                        <td>
                                            <span class="ph#" style="color: gray;font-size: 17px" >{{$order->email}}</span>
                                        </td>
                                         
                                        <td>
                                            <span class="addr" style="color: gray; font-size: 17px">{{$order->phone}}</span>
                                        </td>
                                        
                                       <td>
                                            <h4 class="name" style="color: gray; width: 100px;font-size: 17px">{{$order->church}}</h4>
                                        </td>
                                        <td>
                                            <?php 
                                            //Explode array using the separating comma to convert string to an array 
                                             ...
                                             @foreach($exploded_services as $services)
                                                ...
                                                //Fetch product details from database based on the id from array string
                                                  ...
                                                  ?>
                                                <span class="addr alert alert-info" style="font-size: 17px">{{$order_details->title}}</span> <br />
                                            @endforeach

                                        </td>
                                         <td>
                                            <span class="ph#" style="color: gray;font-size: 17px" >{{$order->amountbox}}</span>
                                        </td>
                                        <td>
                                            <span class="ph#" style="color: gray;font-size: 17px" >{{$order->currency}}</span>
                                        </td>
                                        <td>
                                        <span class="date" style="color: gray; font-size: 17px">{{$order->status}}</span>&nbsp;&nbsp;
                                            <i class="fa fa-clock-o" aria-hidden="true" style="color: gray"></i>
                                        </td>
                                        <td>
                                            <span class="addr" style="color: gray; font-size: 17px">{{$order->created_at}}</span>
                                        </td>
